...
- **Model-parallel runtime dispatch** (`core-host/src/ai_inference/candle_llm_runtime.rs`): the tensor/pipeline/expert-parallel engines are now selected by the live model loader. `IntegrityModelBinding` carries a `hardware_strategy` (mirroring `wit/config-ai.wit`'s `hardware-strategy`), and `try_load` validates the requested plan against the discovered hardware topology before constructing the matching engine. Tensor-parallelism runs the full autoregressive decode loop; pipeline-parallelism is loaded and prefill-correct (its decode loop is a follow-up, so generation returns a typed error until then); expert-parallelism is validated and placed but rejected at load until a full MoE checkpoint loader exists. The `nvfp4-cuda` feature now pulls `candle-cuda` (`candle-core/cuda`), so the CUDA build enumerates real GPUs and uses NCCL instead of the CPU-staged all-reduce; the default build stays CPU-only and CI-reproducible. See `openspec/changes/2026-06-22-wire-model-parallel-runtime-dispatch`.
