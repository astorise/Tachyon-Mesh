...
export class TachyonBundleConflictModal extends HTMLElement {
  ...
  private render(): void {
    ...
    this.root.innerHTML = `
      <div class="fixed inset-0 z-[110] flex items-center justify-center bg-slate-950/85 backdrop-blur-sm" role="dialog" aria-modal="true" aria-labelledby="conflict-modal-title">
        <div class="w-[min(40rem,calc(100vw-2rem))] rounded-lg border border-amber-500/40 bg-slate-900 p-5 shadow-[0_0_28px_rgba(251,191,36,0.18)]" id="conflict-modal-panel">
          ...
          <ul id="conflict-rows" class="space-y-2 mb-4" aria-label="${t("bundle.conflict.list-label")}"></ul>
    ...
    // Populate the dynamic conflict rows using DOM API (user data via textContent).
    const rowsHost = this.root.getElementById("conflict-rows");
    if (rowsHost) {
      rowsHost.replaceChildren(
        ...this.conflicts.map((conflict) => {
          ...
          return el("li", {
            class: "rounded border border-slate-800 bg-slate-950/50 p-3 space-y-2",
            "data-conflict": conflict.name,
          },
            el("div", { class: "flex items-center justify-between text-xs" },
              el("span", { class: "font-mono text-cyan-300" }, conflict.name),
              el("span", { class: "text-slate-400" },
                t("bundle.conflict.bundled"), " ",
                el("span", { class: "font-mono text-slate-200" }, conflict.bundledVersion),
                " · ", t("bundle.conflict.cluster"), " ",
                el("span", { class: "font-mono text-slate-200" }, conflict.clusterVersion),
              ),
            ),
            el("div", { class: "grid grid-cols-2 gap-2 text-[11px]" },
              el("button", {
                "data-action": "cluster",
                "data-name": conflict.name,
                class: "rounded border border-cyan-500/40 bg-cyan-500/10 px-2 py-1 text-cyan-200 hover:bg-cyan-500/20" + clusterActive,
              }, t("bundle.conflict.use-cluster")),
              el("button", {
                "data-action": "local",
                "data-name": conflict.name,
                class: "rounded border border-amber-500/40 bg-amber-500/10 px-2 py-1 text-amber-200 hover:bg-amber-500/20" + localActive,
              }, t("bundle.conflict.force-local")),
            ),
          );
        }),
      );
