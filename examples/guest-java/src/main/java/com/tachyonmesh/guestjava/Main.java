...
public final class Main {
    ...
    private static void drainStdin() throws IOException {
        ...
        byte[] ioVecStorage = new byte[Structure.sizeOf(IOVec.class)];
        byte[] sizeResultStorage = new byte[Structure.sizeOf(SizeResult.class)];
        IOVec ioVec = Address.ofData(ioVecStorage).toStructure();
        ioVec.buffer = Address.ofData(buffer);
        ioVec.bufferLength = buffer.length;
        SizeResult bytesRead = Address.ofData(sizeResultStorage).toStructure();
    ...
    private static void writeStdout(byte[] bytes) throws IOException {
        ...
        SizeResult bytesWritten = Address.ofData(sizeResultStorage).toStructure();
