...
fn secrets_scope_allows_matching_name() {
    let scopes = DeploymentScopes::from_manifest(&serde_json::json!({ "secrets": ["db/prod/*"] }))
        .expect("manifest should parse");
    assert!(
        scopes.check_secrets("db/prod/password"),
        "glob must match sub-path"
    );
    assert!(
        scopes.check_secrets("db/prod/p"),
        "short name under glob must match"
    );
}
...
fn secrets_scope_denies_non_matching_name() {
    ...
    assert!(
        ...
        "different prefix must not match"
    );
    assert!(
        !scopes.check_secrets("db/staging/pw"),
        "different branch must not match"
    );
}
...
fn secrets_absent_category_denies_all() {
    ...
    let scopes = DeploymentScopes::from_manifest(&serde_json::json!({ "kv": ["t/*"] }))
        .expect("manifest should parse");
    assert!(
        !scopes.check_secrets("db/prod/pw"),
