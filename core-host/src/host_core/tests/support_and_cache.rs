...
#[derive(Clone, PartialEq, Message)]
pub(super) struct TestGrpcHelloResponse {
    #[prost(string, tag = "1")]
    pub(super) message: String,
}

pub(super) fn expected_secret_status() -> &'static str {
...
pub(super) fn expected_guest_example_body(payload: &str) -> String {
    format!(
...
#[test]
pub(super) fn l4_bind_address_preserves_host_and_sets_port() {
...
#[test]
pub(super) fn l7_domain_route_lookup_uses_normalized_domains() {
...
pub(super) fn generate_mtls_test_material() -> MtlsTestMaterial {
...
pub(super) fn encode_test_grpc_message<T>(message: &T) -> Vec<u8>
where
    T: Message,
{
...
pub(super) fn decode_test_grpc_message<T>(payload: &[u8]) -> T
where
    T: Message + Default,
{
    assert!(
...
pub(super) fn test_log_sender() -> mpsc::Sender<AsyncLogEntry> {
    disconnected_log_sender()
}

pub(super) fn test_route_overrides() -> Arc<ArcSwap<HashMap<String, String>>> {
    Arc::new(ArcSwap::from_pointee(HashMap::new()))
}

pub(super) fn test_peer_capabilities() -> PeerCapabilityCache {
    Arc::new(Mutex::new(HashMap::new()))
}

pub(super) fn test_host_load() -> Arc<HostLoadCounters> {
    Arc::new(HostLoadCounters::default())
}

pub(super) fn test_selected_target(module: &str, websocket: bool) -> SelectedRouteTarget {
    SelectedRouteTarget {
...
pub(super) fn build_test_engine(config: &IntegrityConfig) -> Engine {
    build_engine(config, false).expect("engine should be created")
}

pub(super) fn build_test_metered_engine(config: &IntegrityConfig) -> Engine {
    build_engine(config, true).expect("metered engine should be created")
}

#[test]
pub(super) fn compiled_artifact_cache_key_includes_engine_compatibility_hash() {
...
#[test]
pub(super) fn secure_cache_bootstrap_retains_matching_cwasm_cache() {
...
#[test]
pub(super) fn secure_cache_bootstrap_purges_stale_cwasm_cache() {
...
pub(super) fn build_test_runtime(config: IntegrityConfig) -> RuntimeState {
    build_runtime_state(config).expect("runtime state should build")
}

#[test]
pub(super) fn instance_pool_is_isolated_per_runtime_generation() {
...
#[test]
pub(super) fn instance_pool_hits_short_circuit_redb_lookup() {
...
pub(super) fn test_tempdir() -> TestTempDir {
...
pub(super) fn insecure_tls_connector() -> TlsConnector {
...
pub(super) fn signed_manifest(config: &IntegrityConfig, seed: u8) -> IntegrityManifest {
...
pub(super) fn test_host_identity(seed: u8) -> Arc<HostIdentity> {
    Arc::new(HostIdentity::from_signing_key(SigningKey::from_bytes(
        &[seed; 32],
    )))
}

pub(super) fn write_test_manifest(path: &Path, config: &IntegrityConfig, seed: u8) {
...
pub(super) fn autoscaling_test_config(include_background_route: bool) -> IntegrityConfig {
    let mut routes = vec![
...
pub(super) fn sqs_connector_test_config(
...
pub(super) fn gc_batch_target(cache_dir: &Path, ttl_seconds: u64) -> IntegrityBatchTarget {
...
pub(super) fn build_test_state(config: IntegrityConfig, telemetry: TelemetryHandle) -> AppState {
...
pub(super) fn volume_test_route(host_path: &std::path::Path, readonly: bool) -> IntegrityRoute {
    IntegrityRoute {
...
pub(super) fn logger_test_route(host_path: &std::path::Path) -> IntegrityRoute {
    IntegrityRoute {
...
pub(super) fn log_storm_test_route() -> IntegrityRoute {
    let mut route = IntegrityRoute::user("/api/guest-log-storm");
    route.name = "guest-log-storm".to_owned();
    route
}

pub(super) fn execute_legacy_guest_with_sync_file_capture(
    engine: &Engine,
    function_name: &str,
    body: Bytes,
...
pub(super) fn scoped_volume_test_route(
    path: &str,
...
pub(super) fn storage_broker_test_route(host_path: &std::path::Path) -> IntegrityRoute {
    IntegrityRoute {
...
pub(super) fn metering_test_route(host_path: &std::path::Path) -> IntegrityRoute {
    IntegrityRoute {
...
pub(super) fn cert_manager_test_route(host_path: &std::path::Path) -> IntegrityRoute {
    IntegrityRoute {
...
pub(super) fn tcp_echo_test_route(max_concurrency: u32) -> IntegrityRoute {
    IntegrityRoute {
...
pub(super) fn udp_echo_test_route(max_concurrency: u32) -> IntegrityRoute {
    IntegrityRoute {
...
pub(super) fn hibernating_ram_route(host_path: &std::path::Path) -> IntegrityRoute {
    IntegrityRoute {
        path: "/api/guest-volume".to_owned(),
        role: RouteRole::User,
        name: "guest-volume".to_owned(),
...
pub(super) fn ttl_managed_volume_route(
    host_path: &std::path::Path,
    ttl_seconds: u64,
) -> IntegrityRoute {
    IntegrityRoute {
...
pub(super) fn resiliency_test_route(resiliency: Option<ResiliencyConfig>) -> IntegrityRoute {
...
pub(super) fn draining_test_route(module: &str, version: &str) -> IntegrityRoute {
...
pub(super) fn targeted_route(path: &str, targets: Vec<RouteTarget>) -> IntegrityRoute {
    let mut route = IntegrityRoute::user(path);
    route.targets = targets;
    route
}

pub(super) fn versioned_route(path: &str, name: &str, version: &str) -> IntegrityRoute {
    let mut route = IntegrityRoute::user(path);
...
pub(super) fn weighted_target(module: &str, weight: u32) -> RouteTarget {
    RouteTarget {
        module: module.to_owned(),
        weight,
        websocket: false,
...
pub(super) fn header_target(module: &str, header_name: &str, header_value: &str) -> RouteTarget {
    RouteTarget {
        module: module.to_owned(),
        weight: 0,
        websocket: false,
...
pub(super) fn websocket_target(module: &str) -> RouteTarget {
    RouteTarget {
        module: module.to_owned(),
        weight: 100,
        websocket: true,
...
pub(super) fn capability_target(module: &str, requires: &[&str]) -> RouteTarget {
...
pub(super) fn system_targeted_route(path: &str, module: &str) -> IntegrityRoute {
    let mut route = IntegrityRoute::system(path);
    route.targets = vec![weighted_target(module, 100)];
    route
}

pub(super) fn route_env(entries: &[(&str, &str)]) -> BTreeMap<String, String> {
    entries
        .iter()
...
pub(super) fn mounted_volume(host_path: &Path, guest_path: &str) -> IntegrityVolume {
    IntegrityVolume {
        volume_type: VolumeType::Host,
        host_path: host_path.display().to_string(),
        guest_path: guest_path.to_owned(),
...
pub(super) fn mounted_ram_volume(host_path: &Path, guest_path: &str) -> IntegrityVolume {
    IntegrityVolume {
        volume_type: VolumeType::Ram,
        host_path: host_path.display().to_string(),
        guest_path: guest_path.to_owned(),
