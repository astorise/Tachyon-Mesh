        ...
        let mut advertisements = self
            ...
            .filter_map(|model| {
                model
                    .backend_model
                    .as_any()
                    .downcast_ref::<CandleBackendModel>()
                    .and_then(|backend| match &backend.kind {
                        CandleBackendModelKind::Magnetar(runtime) => {
...
enum CandleBackendModelKind {
    ...
    Magnetar(Box<magnetar_runtime::MagnetarRuntime>),
...
impl CandleBackendModel {
    ...
    fn load(binding: &IntegrityModelBinding) -> Result<Self> {
        ...
        } else if let Some(runtime) = magnetar_runtime::MagnetarRuntime::try_load(
            &binding.alias,
            &binding.path,
            binding.device.as_str(),
        )? {
            CandleBackendModelKind::Magnetar(Box::new(runtime))
        } else if magnetar_runtime::is_magnetar_path(&binding.path) {
            return Err(anyhow!(
...
impl BackendModel for CandleBackendModel {
    fn residency(&self) -> AcceleratorMemoryResidency {
        if matches!(self.kind, CandleBackendModelKind::Magnetar(_)) {
            return AcceleratorMemoryResidency::MagnetarArena;
        }
            ...
            CandleBackendModelKind::Magnetar(runtime) => {
                ...
                let result = runtime
                    ...
                    .map(|outputs| {
                        outputs
                            .into_iter()
                            .map(|(bytes, usage)| InferenceOutput::measured(bytes, usage, None))
                            .collect::<Vec<_>>()
                    })
                    .map_err(|error| {
                        anyhow!(
                            "Magnetar model `{}` loaded from `{}` failed: {error}",
                            self.source.alias,
    ...
    fn execute_with_adapter(
        ...
        match &self.kind {
            ...
            CandleBackendModelKind::Magnetar(_)
...
mod tests {
    ...
    fn write_qwen_safetensors_fixture(path: &std::path::Path) {
        fs::create_dir_all(path).expect("fixture dir should be created");
        fs::write(path.join("config.json"), br#"{"model_type":"qwen3"}"#)
            .expect("config should be written");
        fs::write(path.join("model.safetensors"), b"weights").expect("weights should be written");
    }

    #[test]
    fn magnetar_qwen_binding_loads_through_runtime_and_advertises_capabilities() {
        let model_dir = unique_candle_llm_dir("magnetar-qwen-runtime");
    ...
    fn explicit_magnetar_binding_rejects_non_qwen_model_directory() {
        let model_dir = unique_candle_llm_dir("magnetar-non-qwen");
        fs::create_dir_all(&model_dir).expect("fixture dir should be created");
        fs::write(model_dir.join("config.json"), br#"{"model_type":"llama"}"#)
            .expect("config should be written");
        fs::write(model_dir.join("model.safetensors"), b"weights")
            .expect("weights should be written");
        let error = match CandleBackendModel::load(&IntegrityModelBinding {
