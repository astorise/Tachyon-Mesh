...
## 5. Correction made during implementation: cache stays external, not a `PipelineStage` field

§2.1-2.3 above (the originally-authored design) proposed storing `TensorParallelCache` as a mutable field on `PipelineStage` and converting `run_stage`/`PipelineStageExecutor` to `&mut self`. Implementation deviated from this for a concurrency-correctness reason discovered by comparing against the already-proven `tensor_parallel_llama.rs` pattern before writing the new code:

`CandleLlmRuntime` holds its loaded model behind `Arc<LoadedModel>`, which can be shared across **concurrent** generation requests (the QoS scheduler may dispatch more than one request against the same loaded model). `TensorParallelLlama`/`TensorParallelBlock`/`ReplicatedAttention` all deliberately keep `forward(&self, ...)` immutable on the model itself; `TensorParallelCache` is built fresh, as a local variable, once per request, inside `decode()`'s `ParallelModel::Tensor` arm, and captured by `&mut` in the per-step closure. If `PipelineStage` instead stored its cache as a field (as originally designed) and `run_stage` took `&mut self`, two concurrent requests against the same `Arc<PipelineParallelLlama>` would need `&mut` access to the same stage at the same time — impossible to do safely without a `Mutex` serializing every stage's every decode step across all concurrent requests, which `TensorParallelLlama` does not need and pipeline parallelism should not need either.

**What was actually built instead** (implemented, tested, see `tasks.md`):
- `PipelineStage` stays free of any cache field; it gains `PipelineStage::new_cache(&self) -> CandleResult<TensorParallelCache>` (builds a fresh, decode-capable cache on demand) and `PipelineStage::forward_with_cache(&self, index_pos, input, cache: &mut TensorParallelCache)` (stays `&self`, cache passed in by the caller).
- `PipelineParallelLlama::new_caches(&self) -> CandleResult<Vec<TensorParallelCache>>` builds one cache per stage, once per generation request (mirroring exactly where `TensorParallelCache::new` is called in `candle_llm_runtime.rs`'s `ParallelModel::Tensor` arm), and `PipelineParallelLlama::forward_at(&self, index_pos, tokens, transports, caches: &mut [TensorParallelCache])` threads those caches through every stage's `forward_with_cache` call.
- `PipelineStageExecutor::run_stage`'s trait signature was **not** changed (stays `&self`, no `index_pos`) — it now delegates to `forward_with_cache(0, input, &mut self.new_cache()?)`, preserving its exact prior behavior (fresh, throwaway cache, position 0 always) for the still-prefill-only `run_pipeline`/`run_pipeline_microbatched` schedulers and the existing TCP-transport test, none of which needed to change.

This achieves the same end goal as §2.1-2.3 (a persistent cache across decode steps within one generation request) through a mechanism consistent with the rest of the codebase's concurrency model, while also reducing the blast radius: `parallel.rs`'s trait and schedulers are untouched, and the deviation from Tasks 1/2's literal wording is documented in `tasks.md`.

§3's `StageTransport` connection-lifetime concern did not need resolving: the production decode wiring in `candle_llm_runtime.rs` only ever constructs `InProcessTransport` (the same same-process composition the prefill path already used), which has no connection state to amortize. Real cross-node decode over `TcpStageTransport` remains future work, exercised only by the existing single-call TCP test.
