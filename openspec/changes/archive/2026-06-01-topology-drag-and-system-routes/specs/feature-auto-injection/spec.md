...
AI bundle SHALL include `/system/model-broker`, `/system/ai-list-model`, **and**
`/system/ai-openai-adapter`.

#### Scenario: ai-inference binary activates AI routes automatically
- **WHEN** `core-host` is built with `--features ai-inference` and starts with a manifest that does not contain `/system/model-broker`
- **THEN** the runtime contains `/system/model-broker`, `/system/ai-list-model`, and `/system/ai-openai-adapter` as `role=system` routes
