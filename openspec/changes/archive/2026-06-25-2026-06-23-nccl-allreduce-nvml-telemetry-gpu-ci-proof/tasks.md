...
- [x] **Task 5: GPU-proof CI step**
  ...
  - **Independently re-verified against a live `arc-gpu-runners` run**: `cuda-quality` job `83002134742` of workflow run `28039605008` (triggered by the `push` to `main` that merged this change in PR #221, 2026-06-23T16:09Z) shows `test ai_inference::parallel::tests::nccl_all_reduce_matches_cpu_staged_reference ... ok` against real GPU hardware, job conclusion `success`. Confirmed via `mcp__github__get_job_logs`.

- [x] **Task 6: Tests**
  - `cargo test -p core-host --features ai-inference parallel::` — done, 0 regressions (18/18 passed).
  - `cargo test -p core-host --features candle-cuda ...` on real CUDA hardware — confirmed via the `cuda-quality` run above: `nccl_all_reduce_matches_cpu_staged_reference ... ok` on real GPUs.
  - `cargo clippy -p core-host --features candle-cuda ...` — confirmed via the same job (`cargo clippy --features candle-cuda -- -D warnings -D clippy::unwrap_used` step), job conclusion `success`; `cargo clippy -p core-host --features ai-inference --all-targets -- -D warnings -D clippy::unwrap_used` (the non-CUDA build) was also run locally and is clean.
