...
- [x] Update the `core-host` storage driver to emit an async event upon successful writes to flagged resources.
...
## Phase 4: Validation
- [x] **Offline Test:** Disconnect the machine from the internet. Write data to the `sync_to_cloud` enabled FaaS. Verify local reads are instant and the CDC local spool grows.
- [x] **Sync Test:** Reconnect the internet. Verify the CDC module detects the connection, pushes the data to the mock Cloud endpoint, and clears its local spool.
