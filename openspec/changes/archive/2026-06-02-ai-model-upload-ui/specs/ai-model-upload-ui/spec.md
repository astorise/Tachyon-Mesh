...
On success the panel SHALL display the returned asset reference and indicate that the model will appear in the model list (`GET /v1/models`); on failure it SHALL display the translated backend error.
...
- **AND** the panel indicates the model is being registered and will appear in the model list (`/v1/models` via `guest-openai`)
