...
- [x] 4.1 Added a `cuda-quality` step ("Run single-device Llama CUDA execution proof") running Task 3.2's test on `arc-gpu-runners`, right after the existing NCCL proof step. YAML validated with a `yaml.safe_load` parse; not yet exercised by an actual CI run.
