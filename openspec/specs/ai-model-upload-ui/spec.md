# ai-artifact-upload-ui Specification

## Purpose
TBD - created by archiving change ai-artifact-upload-ui. Update Purpose after archive.
## Requirements
### Requirement: Operator can upload a model file from the UI
The Tachyon UI SHALL provide a `<tachyon-artifact-upload-panel>` control, mounted in the AI Orchestration view, that lets an operator select a local model file and upload it to the cluster by invoking the `push_large_model` command with the selected file path.
...
- **WHEN** the operator triggers a artifact upload
...
The artifact-upload panel SHALL be rendered only within the AI Orchestration view, which is shown when `has_ai` is true. Model upload depends on an active `model-broker` (an `ai-inference` build); when the broker is absent the upload SHALL surface the broker's backend error through the normal error path.
