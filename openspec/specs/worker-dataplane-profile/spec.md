# worker-dataplane-profile Specification

## Purpose
Defines the `admin-plane` Cargo feature that lets `core-host` be built as a worker/data-plane-only node with no `/admin/*` HTTP surface, while remaining enrollable and dead-code-clean under the CI feature matrix.
