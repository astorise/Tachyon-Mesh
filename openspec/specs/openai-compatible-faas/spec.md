...
### Requirement: Streaming chat completions deliver fragments before generation ends

`guest-openai` SHALL support `stream: true` on `/v1/chat/completions`. It SHALL
acquire the host streaming sink (`tachyon:mesh/response-body`) set up by the
streaming HTTP transport, commit `content-type: text/event-stream` headers via
`begin`, pull decoded fragments from the CPU accelerator's `compute-stream`
resource, and emit one `chat.completion.chunk` SSE frame per fragment. A
terminating `data: [DONE]` frame SHALL be written after the last fragment. The
concatenation of the `delta.content` fields SHALL equal the `message.content`
field of the equivalent non-streamed response. The OpenAI SSE framing lives in
`guest-openai` (user FaaS); the host provides only the generic body-flush
transport.

#### Scenario: Streaming response terminates with [DONE]

- **WHEN** a streaming request is processed successfully
- **THEN** the last SSE line is `data: [DONE]`
- **AND** the connection closes after that frame

#### Scenario: Streamed deltas equal the buffered content

- **GIVEN** the same prompt run with `stream: false` producing content `C`
- **WHEN** the same prompt is run with `stream: true`
- **THEN** the concatenation of all `delta.content` values equals `C`
