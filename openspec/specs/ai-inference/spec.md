...
### Requirement: The streaming transport delivers time-to-first-token over HTTP
...
body bytes with `write`; each write is forwarded to the client immediately via
an axum `Body::from_stream` backed by a bounded tokio channel. When the guest
never calls `begin`, the execution falls back to a buffered response whose
headers and body are forwarded through the same channel pair after
`handle-request` returns.

#### Scenario: Streaming response flushes chunks in real time

- **GIVEN** a `guest-openai` component that calls `get-streaming-response` and `begin`
- **WHEN** the guest writes SSE chunks via `write`
- **THEN** each chunk is immediately forwarded to the connected HTTP client
- **AND** the client receives the `content-type: text/event-stream` header before any body

#### Scenario: Non-streaming request falls back to buffered delivery

- **WHEN** a request does not carry `Accept: text/event-stream`
- **THEN** the host routes it through the existing buffered `execute_component_guest` path
- **AND** the `streaming-response` slot is absent; `get-streaming-response` returns an error
