...
function isExampleRoute(route) {
    ...
    // `guest-openai` is the OpenAI-compatible API surface (/ai/v1/models,
    // /ai/v1/chat/completions, /internal/guest-openai/*), not a demo. Keep it
    // sealed even when stripping the other example guest-* routes, otherwise
    // `GET /ai/v1/models` 404s and uploaded models never appear in the list.
